      ...
      $(document).ready(function(){
        ...
        table = $("#table_utilisateur").DataTable({ 
          ...
          "order": [],
          "ajax": {
            "url": "http://localhost/rivoproject/sysdba/utilisateur/ajax_list/",
            "type": "POST"
          },
          "columnDefs": [
            { 
      ...
      function delete_utilisateur(id)
      {
        ...
        {
          $.ajax({
              url : "http://localhost/rivoproject/sysdba/utilisateur/ajax_delete/"+id,
              type: "POST",
              dataType: "JSON",
              success: function(data)
              {
      ...
      function filtrer()
      {
        ...
        //if(ville!=-1 && ville!=-1)
        table.ajax.url("http://localhost/rivoproject/sysdba/utilisateur/getUsersFiltre/"+ville+"/"+etat+"/"+niveau).load();
      }

      function display_cin(fileName)
      {
        var url = "http://localhost/rivoproject/uploads/cin/"+fileName;
        $('#imgcin').attr('src',url);
        $('#cinmodal').modal('show');
        //alert(fileName);
      }
      ...
      {
        var url = "http://localhost/rivoproject/uploads/dip/"+fileName;
        $('#imgcin').attr('src',url);
        $('#cinmodal').modal('show');
        //alert(fileName);
      }
      ...
      {
        $.ajax({
              url : "http://localhost/rivoproject/sysdba/utilisateur/validation/"+id,
              type: "POST",
              dataType: "JSON",
              success: function(data)
              {
      ...
      {
        $.ajax({
              url : "http://localhost/rivoproject/sysdba/utilisateur/supvalidation/"+id,
              type: "POST",
              dataType: "JSON",
              success: function(data)
              {
